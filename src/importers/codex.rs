...
fn import_with_modified_since(
    ...
    let source_id = db.record_import_source("codex", source_kind, &root_path)?;
...
fn parse_session_line(
    ...
    if is_user_turn_boundary(outer_type, payload_type, &payload) {
        ...
        return Ok(vec![state.event(
            ...
            line_number,
            timestamp_ns,
            format!("{}:user:{}", state.run_external_id(), line_number),
            OperationType::User,
    ...
    match payload_type {
        "token_count" => Ok(vec![
            state.event(
                ...
                line_number,
                timestamp_ns,
                format!("{}:llm:{line_number}", state.run_external_id()),
